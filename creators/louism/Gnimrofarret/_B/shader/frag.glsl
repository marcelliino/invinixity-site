...
//		uv.x += UV.x < 1.0 ? 1.0 : -2.0;
    ...
    vec2 mask = (st / 0.5 - 1.0);
    mask = vec2(length(mask * mask));
    col.rgb *= smoothstep(1.0, 0.5, mask.x);
    
    col.rgb = smoothstep(-0.0625, 0.25, col.rgb);
